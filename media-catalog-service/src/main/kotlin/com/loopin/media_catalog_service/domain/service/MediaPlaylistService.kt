...
class MediaPlaylistService(
        ...
        mediaPlaylistRepository
            ...
            .flatMap { playlist ->
                mediaItemWithPositionRepository
                    ...
                    .map {
                        logger.info(
                            "Found ${it.id} ${it.title} ${it.playlistPosition} ${it.playlistRankKey}"
                        )
                        it.toDto(
                            position = it.playlistPosition,
                            rankKey = it.playlistRankKey
                        )
                    }
                    .collectList()
    ...
    fun createByResourceId(resourceId: String): Mono<MediaPlaylist> =
        mediaPlaylistRepository
            ...
            .flatMap {
                Mono.error<MediaPlaylist>(
                    AlreadyExistsException("playlist already exists for $resourceId")
                )
            }
            .switchIfEmpty(
                youtubeClient.getPlaylistWithItems(resourceId).flatMap { youtubeData ->
                    //                        logger.info(youtubeData.toString())
                    logger.debug(
                        "Saving playlist ${youtubeData.playlist.resourceId} ${youtubeData.playlist.title}"
                    )

                    mediaPlaylistRepository
                        .findByResourceId(youtubeData.playlist.resourceId)
                        .switchIfEmpty(
                            mediaPlaylistRepository.save(youtubeData.playlist)
                        )
                        .flatMap { playlist ->
                            logger.debug(
                                "Saving ${youtubeData.mediaItem.size} items for playlist ${playlist.id} ${playlist.resourceId} ${playlist.title}"
                            )

                            playlistItemMappingRepository
                                .findFirstByPlaylistIdOrderByRankKeyAsc(
                                    playlist.id!!
                                )
                                .map { it.rankKey }
                                .defaultIfEmpty("")
        ...
        youtubeClient.getPlaylistWithItems(resourceId).flatMap { youtubeData ->
            ...
            mediaPlaylistRepository
                ...
                .switchIfEmpty(
                    Mono.error(
                        NotExistsException(
                            "Playlist with resourceId $resourceId not found"
                        )
                    )
                )
                .flatMap { current ->
                    logger.debug(
                        "Updating playlist ${current.id} ${current.resourceId} ${current.title}"
                    )

                    /* ───────────────────── 1. 메타데이터가 바뀌면 갱신 ───────────────────── */
                    val metaChanged =
                        current.title != youtubeData.playlist.title ||
                                current.description !=
                                youtubeData.playlist.description ||
                    ...
                    val firstKeyMono: Mono<String?> =
                        firstKeyMonoRow.mapNotNull { it.ifEmpty { null } }
                    ...
                    val deleteOldMappingsMono: Mono<Void> =
                        playlistItemMappingRepository
                            .findByPlaylistId(current.id)
                            .flatMap { mapping ->
                                mediaItemRepository.findById(mapping.mediaItemId)
                                    .flatMap { item ->
                                        if (!newResIdSet.contains(
                                                item.resourceId
                                            )
                                        ) {
                                            playlistItemMappingRepository
    ...
    fun getSlice(
        ...
        mediaPlaylistRepository.findAllBy(
            size = size,
            sortBy = sortBy,
            direction = direction,
            offset = offset,
    ...
    fun getById(id: Long): Mono<MediaPlaylist> =
        mediaPlaylistRepository
            .findById(id)
            .switchIfEmpty(Mono.error(NotExistsException("Playlist $id not found")))

    fun deleteById(id: Long): Mono<Void> =
        mediaPlaylistRepository
            .findById(id)
            .switchIfEmpty(Mono.error(NotExistsException("Playlist $id not found")))
            .flatMap { mediaPlaylistRepository.delete(it) }

    fun deleteByResourceId(resourceId: String): Mono<Void> =
        mediaPlaylistRepository
            .findByResourceId(resourceId)
            .switchIfEmpty(
                Mono.error(
